...
def add_footer(fig: plt.Figure) -> None:
    fig.text(
        0.995,
        0.003,
        "Certified · independent raw-dump verification · 563 artifacts",
        ha="right",
        va="bottom",
        fontsize=5.8,
        color=GRAY,
...
def figure_f7_1a(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    effects = np.array([row["singleton_inference"][name]["effect"] for name in names])
    intervals = np.array([row["singleton_inference"][name]["ci95"] for name in names])
    y = np.arange(4)[::-1]
    ...
    ax.axvline(row["certified_bias_off_cost"], color=VERMILLION, linestyle="--", linewidth=1.2)
    ax.text(
        row["certified_bias_off_cost"],
        3.35,
        "full L29 bias-off",
        color=VERMILLION,
        ha="right",
    ...
    ax.set_xlabel("Mean ΔNLL after removing one signed distance cell")
    ax.set_title("F7-1a · The causal near field collapses to d = 0", loc="left")
    fraction = row["costs"]["d0_off_L29"] / row["certified_bias_off_cost"]
    ax.annotate(
        f"d = 0 alone\naccounts for {fraction:.1%}",
        xy=(effects[0], y[0]),
        xytext=(0.061, 1.55),
...
def figure_f7_1b(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    bars = axes[0].barh(ablation_labels[::-1], ablation[::-1] * 100, color=[GRAY, BLUE], height=0.55)
    axes[0].axvline(100, color=VERMILLION, linestyle="--", linewidth=1.0)
    axes[0].set_xlabel("Damage reproduced (% of full bias-off)")
    axes[0].set_xlim(0, 108)
    axes[0].set_title("Ablate cells", loc="left")
    annotate_bars(axes[0], bars, lambda value: f"{value:.1f}%")
    clean_axis(axes[0], "x")
    ...
    bars = axes[1].barh(rescue_labels[::-1], rescue[::-1] * 100, color=[TEAL, GRAY, BLUE], height=0.55)
    axes[1].axvline(100, color=INK, linestyle=":", linewidth=1.0)
    axes[1].set_xlabel("Damage rescued (% of full bias-off)")
    axes[1].set_xlim(0, 108)
    axes[1].set_title("Restore or retain cells", loc="left")
    annotate_bars(axes[1], bars, lambda value: f"{value:.1f}%")
    clean_axis(axes[1], "x")
    panel_label(axes[1], "b")
    fig.suptitle("F7-1b · d = 0 is necessary and nearly sufficient", x=0.01, ha="left")
...
def figure_f7_2a(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    ax.set_title("F7-2a · L29-containing removals are strongly super-additive", loc="left")
...
def figure_f7_2b(_results: dict[str, Any], derived: dict[str, Any]) -> None:
    ...
    fig, ax = plt.subplots(figsize=(7.4, 4.3), layout="constrained")
    annotated_heatmap(
        ax,
        data,
    ...
    ax.set_title("F7-2b · Backup strength is text-dependent but positive everywhere", loc="left", pad=17)
...
def figure_f7_3a(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    ax.set_xlim(-2, 106)
    ax.set_xlabel("Causal cost (% of full L29 bias-off)")
    ax.set_title("F7-3a · Static non-carrier mean carries the causal r-object", loc="left")
    annotate_bars(ax, bars, lambda value: f"{value:.1f}%")
...
def figure_f7_3b(_results: dict[str, Any], derived: dict[str, Any]) -> None:
    ...
    norm = mcolors.TwoSlopeNorm(vmin=-max(0.01, vmax * 0.08), vcenter=0.0, vmax=vmax)
    ...
    annotated_heatmap(
        ...
        norm=norm,
        fmt="+.3f",
        cbar_label="Mean ΔNLL",
    )
    ax.set_title("F7-3b · The static-mean mechanism generalizes, except on the needle mixture", loc="left", pad=17)
...
def figure_f7_4a(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    labels = ["Q1 · highest stencil score", "Q2", "Q3", "Q4"]
    colors = [BLUE, LIGHT_GRAY, LIGHT_GRAY, LIGHT_GRAY]
    fig, ax = plt.subplots(figsize=(7.2, 4.2), layout="constrained")
    bars = ax.bar(labels, values, color=colors, edgecolor=[BLUE, MID_GRAY, MID_GRAY, MID_GRAY], linewidth=0.8)
    ax.set_ylabel("Mean ΔNLL when quartile is removed")
    ax.set_title("F7-4a · Causal load localizes to the frozen top head quartile", loc="left")
    ax.tick_params(axis="x", rotation=13)
    for bar, value in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width() / 2, value + 0.0012, f"{value:.4f}", ha="center", va="bottom", fontsize=7.5)
    ax.set_ylim(0, values.max() * 1.22)
    ax.annotate(
        "Q1 costs 30–67× more",
        xy=(0, values[0]),
        xytext=(1.25, values[0] * 0.78),
...
def figure_f7_4b(_results: dict[str, Any], derived: dict[str, Any]) -> None:
    ...
    ax.fill_between(heads, rescue, 0, color=PALE_BLUE, alpha=0.55, zorder=-3)
    for x, y in zip(heads[1:], rescue[1:]):
        ax.text(x, y + 2.3, f"{y:.1f}%", ha="center", va="bottom", fontsize=7.5, color=BLUE)
    top_ids = ", ".join(str(value) for value in derived["head_order"][:8])
    ax.text(
        0.02,
        0.08,
        f"Frozen top 8 heads: {top_ids}",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
        fontsize=7.2,
    ...
    ax.set_title("F7-4b · Rescue saturates rapidly in the top stencil-ranked heads", loc="left")
...
def figure_f7_5a(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    effects = np.array([record["absolute_rescue"] for record in records])
    intervals = np.array([record["absolute_rescue_ci95"] for record in records])
    y = np.array([1, 0])
    ...
    ax.axvline(0, color=INK, linewidth=1.0)
    ax.set_yticks(y, labels=labels)
    ax.set_xlabel("Absolute needle recall ΔNLL rescued")
    ax.set_title("F7-5a · Restoring only L29 query states rescues retrieval", loc="left")
    for yi, record, color in zip(y, records, [TEAL, GRAY]):
        ax.text(
            max(record["absolute_rescue_ci95"]) + 0.012,
            yi,
...
def figure_f7_5b(_results: dict[str, Any], derived: dict[str, Any]) -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(7.4, 3.9), sharex=True, sharey=True, layout="constrained")
    for ax, patched, title, color in (
        (axes[0], query, "Patch 24 query states", TEAL),
        (axes[1], sham, "Patch 24 sham states", GRAY),
    ):
        ...
        ax.set_xlabel("Bias-off ΔNLL at registered query token")
        clean_axis(ax, "both")
    axes[0].set_ylabel("ΔNLL after patch")
    axes[0].text(0.03, 0.94, "below diagonal = rescue", transform=axes[0].transAxes, color=TEAL, fontsize=7.5, va="top")
    axes[1].text(0.97, 0.06, "★ pooled mean", transform=axes[1].transAxes, color=ORANGE, fontsize=7.5, ha="right")
    panel_label(axes[0], "a")
    panel_label(axes[1], "b")
    fig.suptitle("F7-5b · Token-level mediation is specific to the query positions", x=0.01, ha="left")
...
def figure_f7_6a(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    ax.text(
        0.01,
        -0.25,
        "teal outline = registered pass (< 0.20); only 1/12 cells passes",
        transform=ax.transAxes,
        fontsize=7.5,
        color=TEAL,
    )
    ax.set_title("F7-6a · Five-text clock bases do not transfer to the held-out text", loc="left", pad=17)
...
def figure_f7_6b(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(7.5, 4.4), layout="constrained", gridspec_kw={"width_ratios": [1.15, 0.85]})
    ...
    axes[0].set_xlabel("Median ● to maximum | correlation |")
    axes[0].set_title("Geometry", loc="left")
    ...
    costs = np.array([row["behavior_costs"][name] for name in behavior_order])
    bars = axes[1].barh(behavior_labels[::-1], costs[::-1], color=[TEAL, TEAL, TEAL], height=0.55)
    axes[1].axvline(0.005, color=VERMILLION, linestyle="--", linewidth=1.0, label="null ceiling")
    axes[1].set_xlim(0, 0.0055)
    axes[1].set_xlabel("Mean ΔNLL")
    axes[1].set_title("Behavior at 8k", loc="left")
    annotate_bars(axes[1], bars, lambda value: f"{value:.6f}")
    axes[1].legend(loc="lower right")
    clean_axis(axes[1], "x")
    panel_label(axes[1], "b")
    fig.suptitle("F7-6b · Clock geometry is real; causal cost remains negligible", x=0.01, ha="left")
...
def figure_f7_7a(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(7.6, 3.3), layout="constrained")
    draw_single_effect(
        axes[0],
        rank["effect"],
        rank["ci95"],
    ...
    draw_single_effect(
        ...
        color=ORANGE,
        xlabel="Δ ECE20",
        title="Calibration shifts modestly",
        threshold=(-0.01, 0.01),
    )
    for label, ax in zip("abc", axes):
        panel_label(ax, label)
    fig.suptitle("F7-7a · L29 bias acts primarily through token ranking", x=0.01, ha="left")
...
def class_forest_panel(ax: plt.Axes, rows: list[tuple[str, dict[str, Any]]], title: str, color: str) -> None:
    ...
    for yi, (label, record) in zip(y, rows):
        ...
        ax.errorbar(
            primary,
            yi + 0.11,
            xerr=np.array([[primary - primary_ci[0]], [primary_ci[1] - primary]]),
            fmt="o",
            color=color,
            ecolor=color,
            capsize=3,
            markersize=5,
            label="query-aligned primary" if yi == y[0] else None,
        )
        ax.errorbar(
            sec_value,
            yi - 0.11,
            xerr=np.array([[sec_value - sec_ci[0]], [sec_ci[1] - sec_value]]),
            fmt="o",
            ...
            capsize=3,
            markersize=5,
            label="target-aligned secondary" if yi == y[0] else None,
        )
    ax.axvline(0, color=INK, linewidth=1.0)
    ax.set_yticks(y, labels=[label for label, _ in rows])
...
def figure_f7_7b(results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(7.6, 3.8), layout="constrained")
    class_forest_panel(axes[0], slack, "Fresh Slack", BLUE)
    class_forest_panel(axes[1], math, "Fresh assistant math", ORANGE)
    axes[0].set_xlim(-0.36, 0.36)
    axes[1].set_xlim(-1.35, 2.05)
    panel_label(axes[0], "a")
    panel_label(axes[1], "b")
    fig.suptitle("F7-7b · Boundary/pronoun dependence does not replicate on fresh text", x=0.01, ha="left")
...
def figure_synthesis_a(_results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    ax.set_title("Synthesis A · Revised 8k mechanism: local state construction enables later retrieval", loc="left", pad=8)
...
def figure_synthesis_b(_results: dict[str, Any], _derived: dict[str, Any]) -> None:
    ...
    columns = ["Clause 1", "Clause 2", "Clause 3 / control", "Overall"]
    values = np.array(
        [
            [1, 1, 1, 1],
            [1, 1, 1, 1],
    ...
    labels = np.array(
        [
            ["d=0", "stencil", "d1–3 null", "PASS"],
            ["23+29", "29+35", "triple", "PASS"],
            ["mean", "centered", "carrier null", "PASS"],
            ["Q1", "top-16", "—", "PASS"],
            ["query", "sham", "—", "PASS"],
            ["LOTO", "sham", "behavior null", "FAIL"],
            ["ranking", "calibration", "fresh classes", "FAIL"],
        ],
        dtype=object,
    )
    ...
    for row in range(values.shape[0]):
        for col in range(values.shape[1]):
            ...
            ax.text(col, row, labels[row, col], ha="center", va="center", fontsize=8, color=color, fontweight=weight)
    legend = [
    ...
    ax.set_title("Synthesis B · Registered evidence matrix: five families pass, two split/fail", loc="left", pad=17)
...
FIGURE_INFO: dict[str, dict[str, Any]] = {
    ...
    "f7-3b-r-components-by-text": {
        ...
        "caption": "Raw per-text mean ΔNLL for all six registered r-space interventions. The needle mixture is the clear low-cost exception to the otherwise stable static-mean mechanism.",
        "sources": ["followup r-component token dumps"],
    },
    "f7-4a-head-quartile-localization": {
        "title": "F7-4a — head-quartile localization",
        "caption": "Pooled cost of removing each frozen 16-head quartile. The highest pre-outcome stencil-score quartile costs 30–67 times more than Q2–Q4.",
    ...
    "f7-5a-query-patch-rescue-forest": {
        ...
        "caption": "Absolute needle recall rescue and registered 95% bootstrap intervals. Query patching rescues 54.1%; the seeded sham interval spans zero.",
    ...
    "f7-6b-clock-geometry-behavior-dissociation": {
        ...
        "caption": "Fitted union/per-text bases flatten kernel drift, whereas LOTO transfer fails. All real joint interventions remain far below the registered 0.005 NLL null ceiling.",
